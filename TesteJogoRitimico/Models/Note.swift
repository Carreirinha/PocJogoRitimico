...
class Note: NoteObject{
    ...
    init(position: CGPoint, type: colorType, travelTime: Double) {
        ...
        self.node = SKSpriteNode(imageNamed: type == .blueType ? "DiscoBlue" : type == .pinkType ? "DiscoPink" : type == .blueAndPinkType ? "blueAndPinkNote" : type == .blueUpType ? "blueUpNote" : type == .pinkUpType ? "pinkUpNote" : "blueAndPinkUpNote")
